...
$ node process-2.js 
Hello World!
0: /Users/sanm/.nvm/versions/node/v10.13.0/bin/node
1: /Users/sanm/vscodeProjects/nodeJSTest/src/lesson09.globalObject-test/process-2.js
/Users/sanm/.nvm/versions/node/v10.13.0/bin/node
darwin
```
##### 方法参考手册
...
5 | **getgid()** <br> 获取进程的群组标识（参见 getgid(2)）。获取到得时群组的数字 id，而不是名字。 <br> **注意**：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)。
...
8 | **setuid(id)** <br> 设置进程的用户标识（参见setuid(2)）。接收数字 ID或字符串名字。果指定了群组名，会阻塞等待解析为数字 ID 。<br> **注意**：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)。
...
```
$ node process-3.js 
当前目录: /Users/sanm/vscodeProjects/nodeJSTest/src/lesson09.globalObject-test
当前版本: v10.13.0
{ rss: 20185088,
  heapTotal: 6537216,
  heapUsed: 3852176,
